...
def test_ffdnet(**args):
    ...
    if in_ch == 3:
        model_fn = './logs/BFilter/BFilterColor/80/best_model.pth'
        print("model used Path", model_fn)
    else:
        model_fn = './logs/BFilter/BFilterGray/80/best_model.pth'
    ...
    for f in files_source:
        ...
        # To handle width and height Image size
        if expanded_h:
            imorig = imorig[:, :, :-1, :]
            outim = outim[:, :, :-1, :]
            imnoisy = imnoisy[:, :, :-1, :]
        ...
        if in_ch == 1:
            ...
            model_image = torch.squeeze(imnoisy[:, :3, :, :])
