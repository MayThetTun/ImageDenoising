...
def test_ffdnet(**args):
    ...
    if in_ch == 3:
        model_fn = './logs/OrgColor/Epoch80/best_model.pth'
        print("model used Path", model_fn)
    else:
        model_fn = './logs/OrgGray/Epoch80/best_model.pth'
    ...
    for f in files_source:
        ...
        nsigma = nsigma.to(device)
        outim = torch.clamp(
            imnoisy[:, :3, :, :] - model(imnoisy, nsigma).to(device), 0., 1.)
        end_time = time.process_time()  # single threaded

        # end_time = time.time()  # multiple threaded
        t = end_time - start_time
        print("Elapsed Time: {:.4f} seconds".format(t))
